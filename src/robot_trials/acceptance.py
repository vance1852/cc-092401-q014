...
def run(workspace: Path) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix="robot-trials-") as temporary:
        ...
        try:
            ...
            service.create_user("admin-1", "组织管理员")
            service.create_user("operator-1", "测试操作员")
            service.create_user("stat-1", "统计负责人")
            service.create_user("approver-1", "准入审批人")
            service.create_user("auditor-1", "审计人员")
            service.create_user("org-auditor-1", "组织审计员")
            service.create_org("admin-1", "org-1", "示例事业部")
            service.create_team("admin-1", "org-1", "team-1", "递送试验组")
            for user_id, role in (
                ("operator-1", "operator"),
                ("stat-1", "statistician"),
                ("approver-1", "approver"),
                ...
                service.grant_membership("admin-1", "org-1", "team-1", user_id, role)
            service.grant_org_role("admin-1", "org-1", "org-auditor-1", "org_auditor")
            service.register_robot("operator-1", "org-1", "team-1", "robot-a", "A 型人形机器人", "示例厂商")
            service.register_build("operator-1", "build-a1", "robot-a", "1.0.0", "a" * 64)
            service.publish_protocol("stat-1", "org-1", "team-1", protocol)
            ...
            job = service.claim_job("stat-1", "org-1", "team-1", "worker-1", lease_seconds=60)
            if job is None:
            ...
            report = service.report("org-auditor-1", "batch-demo")
